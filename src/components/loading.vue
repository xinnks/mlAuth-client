...
<template>
  <div v-if="loading">
    <div v-show="loading.show" class="flex space-x-4 items-center border bg-slate-100 border border-slate-400 text-slate-700 px-4 py-4 text-xl font-semibold rounded relative" role="alert">
      <span>
      ...
      <span class="block sm:inline">{{ loading.message }}</span>
    </div>
  </div>
</template>